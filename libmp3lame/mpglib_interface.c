...
int
lame_decoder_init(void)
{
    return InitLayers();
}

void* CDECL lame_decoder_create_context()
{
    return CreateMP3Context();
}

void  CDECL lame_decoder_destroy_context(void* ctx)
{
    DestroyMP3Context((PMPSTR)ctx);
}
...
static int
lame_decode1_headersB_clipchoice(PMPSTR ctx, unsigned char *buffer, int len,
                                                       ...
                                                       int *))
{
    ...
    ret = (*decodeMP3_ptr) (ctx, buffer, len, p, (int) psize, &processed_bytes);
    /* three cases:  
     * 1. headers parsed, but data not complete
     *       ctx->header_parsed==1 
     *       ctx->framesize=0           
     *       ctx->fsizeold=size of last frame, or 0 if this is first frame
     *
     * 2. headers, data parsed, but ancillary data not complete
     *       ctx->header_parsed==1 
     *       ctx->framesize=size of frame           
     *       ctx->fsizeold=size of last frame, or 0 if this is first frame
     *
     * 3. frame fully decoded:  
     *       ctx->header_parsed==0 
     *       ctx->framesize=0           
     *       ctx->fsizeold=size of frame (which is now the last frame)
     *
     */
    if (ctx->header_parsed || ctx->fsizeold > 0 || ctx->framesize > 0) {
        mp3data->header_parsed = 1;
        mp3data->stereo = ctx->fr.stereo;
        mp3data->samplerate = freqs[ctx->fr.sampling_frequency];
        mp3data->mode = ctx->fr.mode;
        mp3data->mode_ext = ctx->fr.mode_ext;
        mp3data->framesize = smpls[ctx->fr.lsf][ctx->fr.lay];

        /* free format, we need the entire frame before we can determine
         * the bitrate.  If we haven't gotten the entire frame, bitrate=0 */
        if (ctx->fsizeold > 0) /* works for free format and fixed, no overrun, temporal results are < 400.e6 */
            mp3data->bitrate = 8 * (4 + ctx->fsizeold) * mp3data->samplerate /
                (1.e3 * mp3data->framesize) + 0.5;
        else if (ctx->framesize > 0)
            mp3data->bitrate = 8 * (4 + ctx->framesize) * mp3data->samplerate /
                (1.e3 * mp3data->framesize) + 0.5;
        else
            mp3data->bitrate = tabsel_123[ctx->fr.lsf][ctx->fr.lay - 1][ctx->fr.bitrate_index];



        if (ctx->num_frames > 0) {
            /* Xing VBR header found and num_frames was set */
            mp3data->totalframes = ctx->num_frames;
            mp3data->nsamp = mp3data->framesize * ctx->num_frames;
            *enc_delay = ctx->enc_delay;
            *enc_padding = ctx->enc_padding;
        }
    }

    switch (ret) {
    case MP3_OK:
        switch (ctx->fr.stereo) {
        case 1:
...
//-----------------------------------------------------------------------------
// New Api, context is used
//-----------------------------------------------------------------------------
int
lame_decode1_ctx_headersB( void* ctx, unsigned char *buffer, int len,
                           short pcm_l[], short pcm_r[], mp3data_struct * mp3data,
                           int *enc_delay, int *enc_padding)
{
    return lame_decode1_headersB_clipchoice((PMPSTR)ctx, buffer, len, (char *) pcm_l, (char *) pcm_r, mp3data,
                                            enc_delay, enc_padding, ((PMPSTR)ctx)->outbuf, OUTBUFFER_SIZE,
                                            sizeof(short), decodeMP3);
}

int
lame_decode1_ctx_headers( void* ctx, unsigned char *buffer, int len,
                          short pcm_l[], short pcm_r[], mp3data_struct * mp3data) 
{
    int     enc_delay, enc_padding;
    return lame_decode1_ctx_headersB( ctx, buffer, len, pcm_l, pcm_r,
