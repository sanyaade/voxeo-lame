...
PMPSTR CreateMP3Context() 
{
    PMPSTR mp = (PMPSTR)malloc(sizeof(MPSTR));
    InitMP3Context( mp );
    return mp;
}

void FreeMP3ContextBuffers( PMPSTR mp )
{
    struct buf *b,*bn;
...
void DestroyMP3Context( PMPSTR mp ) 
{
    FreeMP3ContextBuffers( mp );
    free( mp );
}
